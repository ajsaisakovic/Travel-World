...
const DestinationManagement = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isAddingDestination) {
        ...
        await axios.put(`http://localhost:5000/destination/${editingDestinationId}`, editedDestination);
      }
      const res = await axios.get("http://localhost:5000/destination");
      setDestinations(res.data);
  ...
  const handleAddNewDestination = async () => {
    try {
      ...
      await axios.post("http://localhost:5000/destination", newDestination);
      const res = await axios.get("http://localhost:5000/destination");
      setDestinations(res.data);
      setEditedDestination({});
      setIsAddingDestination(false);
  ...
  const handleDeleteClick = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/destination/delete/${id}`);
      const res = await axios.get("http://localhost:5000/destination");
      setDestinations(res.data);
    } catch (error) {
